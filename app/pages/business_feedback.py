...
def render():
    ...
    if df_shops is not None and not df_shops.empty:
        ...
        if st.button("Enviar Retroalimentación", type="primary"):
            ...
            if not item_id_confirmado:
                ...
                try:
                    ...
                    if resultado == 1:
                        ...
                        st.error("No se pudo guardar la retroalimentación. Verifica conectividad a RDS.")
                except Exception as exc:
                    st.error(f"Error al enviar retroalimentación: {exc}\n\nVerifica conectividad a RDS.")
    ...
    try:
        ...
        if df_feedback.empty:
            st.info("No hay retroalimentación registrada todavía.")
        else:
            st.dataframe(df_feedback, use_container_width=True)

    except Exception as exc:
        st.error(
            f"Error al cargar el listado de retroalimentación: {exc}\n\nVerifica conectividad a RDS."
        )
